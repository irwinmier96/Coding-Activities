...
ALTER TABLE actor
ADD COLUMN _description_ BLOB AFTER last_name;
SELECT * FROM actor;

-- 3b. Very quickly you realize that entering descriptions for each actor is too much effort. Delete the description column.
ALTER TABLE actor
DROP COLUMN _description_;
SELECT * FROM actor;
...
FROM staff
LEFT JOIN address
ON staff.address_id = address.address_id; -- right join returns all the addresses and staff members that are null, left join just staff

-- 6b. Use JOIN to display the total amount rung up by each staff member in August of 2005. Use tables staff and payment.
SELECT staff.staff_id, staff.last_name, staff.first_name, SUM(payment.amount)
FROM staff
LEFT JOIN payment
ON staff.staff_id = payment.staff_id
WHERE staff.last_update LIKE "2006-02%"
